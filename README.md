...
You need to make sure that your MOOSE-app is compiled against `Openmpi/5.0.5` as it might not run otherwise in `CADES/Baseline` otherwise and also wont be compatible with MatEnsemble. 
...
**Note**: You should always check the outputs going othe relevant job directories and finding `stdout` and `stderr` for debugging. Also, it's important to note that a `Failed` job according to MatEnsemble might not have failed from MOOSE perspective, it just means an Error code was picked up (which could be very well due to failure from initial `jit` attempts of the Moose app, while the app could still run fine to produce desired solution output). 